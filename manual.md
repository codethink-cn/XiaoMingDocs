...
如果你使用苹果手机，请寻找使用安卓手机的朋友，按照下面的步骤协助，或参考*第二个方法* 自行抓包。
...
对于 `Windows` 用户，请 **双击** 启动脚本。对于 `Linux` 用户，请在 `小明根目录` 下执行 `sh start.sh` 或 `./start.sh`，启动小明。你很有可能不再遇到安全验证，可以直接进行下一步操作。如果你遇到了安全验证之类操作，请按照 [此前使用过Mirai核心的机器人](#此前未使用过Mirai核心的机器人) 的步骤操作。
